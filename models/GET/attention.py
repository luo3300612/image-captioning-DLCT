...
class ScaledDotProductAttentionGLU(nn.Module):
    '''
    Scaled dot-product attention
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1):
        ...
        :param h: Number of heads
        '''
        super(ScaledDotProductAttentionGLU, self).__init__()
        self.fc_q = nn.Linear(d_model, h * d_k)
        self.fc_k = nn.Linear(d_model, h * d_k)
        self.fc_v = nn.Linear(d_model, h * d_v)
        self.fc_o = nn.Sequential(nn.Linear(h * d_v, 2 * d_model), nn.GLU())
        self.dropout = nn.Dropout(dropout)
    ...
    def init_weights(self):
        ...
        nn.init.constant_(self.fc_k.bias, 0)
        nn.init.constant_(self.fc_v.bias, 0)
        nn.init.constant_(self.fc_o[0].bias, 0)
...
class MultiHeadAttentionGLU(Module):
    '''
    Multi-head attention layer with Dropout and Layer Normalization.
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
        ...
        super(MultiHeadAttentionGLU, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        self.attention = ScaledDotProductAttentionGLU(d_model=d_model, d_k=d_k, d_v=d_v, h=h)
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model)
